...
describe('DeliveryCard', () => {
  ...
  it('показывает поставщика и дату', () => {
    render(<DeliveryCard delivery={mockDelivery()} onEdit={vi.fn()} onDelete={vi.fn()} />)
  ...
  it('кнопка Удалить активна когда нет batch_id', () => {
    render(<DeliveryCard delivery={mockDelivery([null, null])} onEdit={vi.fn()} onDelete={vi.fn()} />)
    const btn = screen.getByText('Удалить').closest('button')
    expect(btn.disabled).toBe(false)
  })

  it('кнопка Удалить неактивна если есть batch_id', () => {
    render(<DeliveryCard delivery={mockDelivery(['b1', null])} onEdit={vi.fn()} onDelete={vi.fn()} />)
    const btn = screen.getByText('Удалить').closest('button')
  ...
  it('вызывает onEdit при клике Изменить', () => {
    ...
    render(<DeliveryCard delivery={delivery} onEdit={onEdit} onDelete={vi.fn()} />)
