...
// ─── getFlowersBySupplier — юнит ──────────────────────────────────

describe('getFlowersBySupplier — фильтрация и дедупликация', () => {
  // Имитируем ответ Supabase: батчи с вложенными flowers
  function mockGetFlowersBySupplier(batches) {
    const seen = new Set()
    return batches
      .filter((row) => {
  ...
  it('возвращает уникальные цветки поставщика', () => {
    const batches = [
      { flower_id: 'fl-1', flowers: { id: 'fl-1', name: 'Роза' } },
      { flower_id: 'fl-2', flowers: { id: 'fl-2', name: 'Тюльпан' } },
      { flower_id: 'fl-1', flowers: { id: 'fl-1', name: 'Роза' } }, // дубликат
    ]
    const result = mockGetFlowersBySupplier(batches)
    assert.equal(result.length, 2)
    assert.equal(result[0].name, 'Роза')
    assert.equal(result[1].name, 'Тюльпан')
  })

  it('не включает цветки других поставщиков (фильтрация на уровне запроса)', () => {
    // sup-1 имеет только Розу
    const batchesSup1 = [
      { flower_id: 'fl-1', flowers: { id: 'fl-1', name: 'Роза' } },
    ]
    const result = mockGetFlowersBySupplier(batchesSup1)
    assert.equal(result.length, 1)
    assert.equal(result[0].id, 'fl-1')
  })

  it('возвращает пустой массив если нет активных партий', () => {
    const result = mockGetFlowersBySupplier([])
